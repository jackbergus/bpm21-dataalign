...
%% Conversion to NNF				
\node (LTLf2) 	[right=2cm of Clause,fill=blue!25,text width=3.5cm,rounded corners=.3cm,draw=black] 
				{$\bot\; \mathcal{R}\; (\neg\texttt{A} \vee (\top\;\mathcal{U}\;(\texttt{B}\wedge \texttt{B}.x>0)))$\\
				 $\top\;\mathcal{U}\;(\texttt{B}\wedge \texttt{B}.x>3\wedge \texttt{B}.y=0)$};
...
\node (LTLf4) 	[right=2cm of LTLf3,fill=blue!25,text width=5cm,rounded corners=.3cm,draw=black] 
				{$\bot\mathcal{R}(\neg\texttt{A}\vee(\top\mathcal{U}(\bigvee_{4\leq i\leq9}p_i)))\wedge\top\mathcal{U} p_8$};

\node (Traces) [fill=red!25,text width=3.9cm,draw=black,above=3cm of Clause,label=above left:{$\mathcal{L}$}] 
			   {\texttt{1:\quad B\{x=1,y=0\}A\{x=6\}A\{x=4\}}\\
				\texttt{2:\quad A\{x=8\}B\{x=10,y=0\}}};

\node (CTraces) [fill=red!25,text width=2cm,draw=black,above=3.2cm of LTLf4] 
                {\texttt{1:\quad $p_5$\;\texttt{A\;A}}\\
                 \texttt{2:\quad A}\;$p_8$};
