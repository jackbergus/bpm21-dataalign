...
\begin{document}
	...
	\node[state] (bot) at ($(SA)!0.5!(ST)-(0,2.5)$) {$s_8$};
	\draw[->] (ST) edge [bend left] node[midway,below,sloped] {$\{\texttt{A}\}$}  (bot)  ; 
	\draw[->] (bot) 
	edge [bend left] node[midway,above,sloped] {$\{p_i|4\leq i\leq 9\}$} (ST)  ;
	\draw (bot) to [in=225,out=315,looseness=8] node[above,right] (W3) {$\Sigma\backslash\{p_i|4\leq i\leq 9\}$} (bot);
	\end{tikzpicture}
\end{document}