...
Given a trace $\sigma$ and a Declare model $\mathcal{M}$, checking the conformance of $\sigma$ against $\mathcal{M}$ is the task of verifying whether $\sigma$ conforms to $\mathcal{M}$, or $\sigma$ is deviant and \added{there} exists a repair sequence $\varrho$ making $\sigma$ non-deviant for $\mathcal{M}$ and guaranteeing a minimal transformation cost.
\end{definition}
